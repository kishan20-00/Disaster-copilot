...
//
// Fully translated, unlike the rest of the app so far: this is the one screen
// where the user is asked to approve something, and an approval gate that asks
// in a language you cannot read is not a gate at all. `draftText` itself is
// already written in the user's language upstream (see gemini.ts / lib/sms.ts).
export function SmsGateModal({ show, smsStatus, draftText, onClose, onApprove }: SmsGateModalProps) {
  const t = useT();
  if (!show) return null;
  return (
      ...
      <div className="bg-white border-t border-slate-200 rounded-t-3xl p-5 pb-[calc(1.25rem+var(--safe-bottom))] space-y-4 animate-in slide-in-from-bottom duration-300">
        ...
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[10px] text-slate-500 font-mono">{t('sms.gate.subtitle')}</span>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="border border-slate-200 rounded-2xl overflow-hidden shadow-inner bg-slate-50 p-4">
          <div className="flex justify-between items-center border-b border-slate-200 pb-2 mb-3">
            <div className="flex items-center gap-1.5 text-[11px] text-slate-500 font-mono">
              ...
              <span>{t('sms.preview.label')}</span>
            </div>
            <span className="text-[10px] bg-indigo-500/10 border border-indigo-500/20 text-indigo-600 px-1.5 py-0.5 rounded font-mono font-extrabold">{t('sms.preview.draft')}</span>
          </div>
          ...
          <div className="mt-3 flex gap-2 text-[10px] text-slate-500">
            <span className="flex items-center gap-1 font-mono"><MapPin className="w-3 h-3 text-indigo-500" /> {t('sms.preview.gpsIncluded')}</span>
          </div>
        </div>
        ...
        {smsStatus === 'idle' && (
          <div className="grid grid-cols-2 gap-3 pt-2">
            ...
            >
              <Copy className="w-3.5 h-3.5" /> {t('sms.copy')}
            </button>
          </div>
        )}
        ...
        {smsStatus === 'copied' && (
          <div className="py-4 flex flex-col items-center justify-center animate-in zoom-in-95 duration-300">
            ...
            <span className="text-xs text-emerald-600 font-bold tracking-tight text-center px-4">{t('sms.copied')}</span>
          </div>
        )}
