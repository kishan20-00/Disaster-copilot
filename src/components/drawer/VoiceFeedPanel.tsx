...
export function VoiceFeedPanel({ currentStep, firstStep, isListening, heardText, sttFeedback, onToggleSpeech }: VoiceFeedPanelProps) {
  return (
    <div className="bg-emerald-50 border border-emerald-300 rounded-2xl p-3.5 space-y-3 animate-in zoom-in-95 duration-200">
      ...
      <div className="border-t border-emerald-300/50 pt-3 flex flex-col gap-2">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border flex items-center gap-1.5 text-[9.5px] font-extrabold uppercase transition-all active:scale-95 duration-200 ${
              isListening
                ...
                : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-700 hover:bg-emerald-500/20 hover:border-emerald-500/40'
            }`}
          >
        ...
        {sttFeedback && (
          <div className="text-[10px] font-medium text-emerald-700 font-mono bg-emerald-500/5 px-2.5 py-1.5 rounded-xl border border-emerald-500/10 animate-in fade-in-50 slide-in-from-top-1 duration-150">
            {sttFeedback}
          </div>
        )}
      </div>
